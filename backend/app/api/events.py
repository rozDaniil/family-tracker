...
from app.models import CalendarLens, Category, Event, EventKind, FamilyProject, Member, User
from app.schemas.common import EventCreateIn, EventOut, EventPatchIn
...
def _verify_lens(db: Session, project_id: UUID, lens_id: UUID | None) -> None:
    if lens_id is None:
        return
    lens = db.get(CalendarLens, lens_id)
    if not lens or lens.project_id != project_id:
...
def list_events(
    ...
    if lens_id:
        _verify_lens(db, project.id, lens_id)
...
def create_event(
    ...
    _verify_lens(db, project.id, payload.lens_id)
...
def patch_event(
    ...
    if "lens_id" in updates:
        _verify_lens(db, project.id, updates["lens_id"])
